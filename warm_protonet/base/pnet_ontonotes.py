...
def snips_reader(
    ...
    if file == "valid.txt" or file == "test.txt":
        ...
        for sentence in sentences:
            ...
            if (
                np.unique(ys_here).shape[0] > 1
                and len(validation_batch) < train_sentences
            ):
        ...
        if (len(validation_all) + len(empty_valid[add_more:])) % (
            total_sentences - train_sentences
        ) == 1:
        ...
        for i in range(int(np.ceil(len(test) / query_number))):
            super_list += copy.deepcopy(
                train
                + test[
                    (i * query_number) : (min(len(test) - 1, (i + 1) * query_number))
                ]
            )
    ...
    if file == "train.txt":
            ...
            for is_divider, lines in itertools.groupby(data_file, _is_divider):
                ...
                if not is_divider:
                    ...
                    valid_sentences.append(
                        [[token, tag] for token, tag in zip(tokens, ner_tags)]
                    )
        np.random.seed(random_seed)
        ...
        for sentence in valid_sentences:
            ...
            if (
                ...
                empty_valid.append(sentence)
        add_more = int(
            (train_sentences / 2) * (len(valid_sentences) / number_appeared - 1)
        )
        add_more = max(50, add_more)
        ...
        ys_sup, counts = np.unique(
            np.concatenate([[xy[1][2:] for xy in sentence] for sentence in sentences]),
            return_counts=True,
        )
        ...
        for episode in range(n_episodes):
            ...
            if np.random.rand() < valid_part:
                ...
                while len(query) < (total_sentences - train_train_sentences):
                    ...
                    ys_here = np.concatenate(
                        [[xy[1] for xy in sentence] for sentence in support]
                    )
                    ys_sup = np.unique(ys_here)
                    # We add our empty sentences to the second 10 sentences to balance the set
                    query_candidate = (
                        validation_batch[(train_sentences // 2) :] + empty_valid
                    )
                ...
                while len(query) < (total_sentences - train_train_sentences):
                    ...
                    support = np.random.choice(
                        support_with_class[class_now],
                        size=train_train_sentences,
                        replace=False,
                    ).tolist()
                    ys_here = np.concatenate(
                        [[xy[1] for xy in sentence] for sentence in support]
